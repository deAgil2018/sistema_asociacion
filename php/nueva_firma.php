                ...
                <div class="block-title">
                    ...
                    <div class="form-group">
                        <div class="col-xs-12">
                            ...
                            <input data-toggle="tooltip" title="Este campo solo permite numeros que empiezen en 2,6 y 7!" type="text" id="firma1" name="firma1" class="form-control" placeholder="Ingrese la firma" autocomplete="off">  
                        </div>  
                    </div>
...
<script src="../js/pages/firma.js?id=<?php echo date('Yidisus') ?>"></script>
<script>
$(function(){ 
    Firma_ingenio.init(); 
    $(document).on("submit", "#registro", function (e) {
        ...
        console.log("el formulario",datos);
            ...
            $.ajax({
                dataType: "json",
                method: "POST",
                url:'json/insertar_firma.php',
                data : datos,
            }).done(function(msg) {
            console.log("esto trae",msg);
            if(msg.exito){

                $.bootstrapGrowl('<h4>Excelente!</h4> <p>la firma ha sido registrada!</p>', {
                    type: "success",
                    delay: 2500,
                    allow_dismiss: true
                });
                ...
                NProgress.done();
                $(location).attr('href','actualizar_firma.php?date=<?php echo date("Yhmsi") ?>&id=<?php echo $_GET[id] ?>');
